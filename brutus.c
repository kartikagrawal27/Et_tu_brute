...
{
  void **myargs = arg_parse(argc, argv);

  pid_t parentid = getpid();
  int numlines = *(int*)myargs[0];
  int maxsize = *(int*) myargs[1];
  FILE * inputfile = myargs[2];
  FILE * outputfile = myargs[3];
  int pipes[numlines][2];
  char *line = NULL;
  ...
  else
  {
    for(int i =0;i<numlines;i++)
    {
      ...
      {
        close(pipes[i][0]);
        char *sigline = lines[i];
        char * decrypted = get_most_likely_print_out(sigline); 
